...
void run_test(const char *ip, int port, int mode, int count) {
    ...
    const char* mode_str = (mode == 1 ? "ASET" : (mode == 2 ? "AGET" : "ADEL"));
...
int main(int argc, char *argv[]) {
    if (argc < 4) {
        printf("Usage: %s <ip> <port> <mode: 1-ASET, 2-AGET, 3-ADEL> [count]\n", argv[0]);
        return -1;
    }
