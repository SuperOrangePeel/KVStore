...
int start_echo(int argc, char *argv[]) {
    struct kvs_loop_s loop;
    int io_uring_entries = 1024;
    if (kvs_loop_init(&loop, io_uring_entries) < 0) {
        return -1;
    }
    ...
    printf("Echo server started on %s:%d\n", net_conf.server_ip, net_conf.port_listen);

    kvs_net_init(&net, &net_conf);
    kvs_loop_run(&loop);

    kvs_net_deinit(&net);

    kvs_loop_deinit(&loop);
