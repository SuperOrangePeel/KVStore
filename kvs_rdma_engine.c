...
kvs_status_t kvs_rdma_post_connect(struct kvs_rdma_engine_s *rdma,  const char *server_ip, int server_port, const void *priv_data, uint8_t priv_len, void* user_data) {
    ...
    rdma_create_id(rdma->event_channel, &rdma->rdma_conn_id, NULL, RDMA_PS_TCP);
    struct rdma_cm_id *conn_id = rdma->rdma_conn_id;

    int ret = rdma_resolve_addr(rdma->rdma_conn_id, NULL, (struct sockaddr *)&server_addr, 2000); // timeout in ms
    if (ret) {
...
int _kvs_rdma_init_conn(struct kvs_rdma_conn_s *conn) {
    ...
    struct ibv_qp_init_attr qp_attr = {
        ...
        .recv_cq = rdma->cq,
        .cap = {
            .max_send_wr = rdma->max_send_wr,
            .max_recv_wr = rdma->max_recv_wr,
            .max_send_sge = rdma->max_sge,
            .max_recv_sge = rdma->max_sge,
        },
        .qp_type = IBV_QPT_RC,
    };
    ...
    id->pd = rdma->pd;
    //LOG_DEBUG("cm_id %p create qp on pd %p", id, id->pd);
    id->recv_cq = rdma->cq;
    id->send_cq = rdma->cq;
    LOG_DEBUG("cm_id %p assign send_cq %p, recv_cq %p", id, id->send_cq, id->recv_cq);
    //conn->header.user_data = NULL;
    return 0;
}
...
int _kvs_rdma_on_route_resolved(struct rdma_cm_id *id) {
    ...
    //LOG_DEBUG("cm_id : %p RDMA route resolved", id);
    struct rdma_conn_param conn_param;
    memset(&conn_param, 0, sizeof(conn_param));
    ...
    while (1) {
        ...
        switch(type) {
            case RDMA_CM_EVENT_ADDR_RESOLVED:
                ...
                //_kvs_rdma_on_error(event->id);
                // if (rdma->callbacks.on_addr_resolved) 
                //     rdma->callbacks.on_addr_resolved(event->id);
            ...
            case RDMA_CM_EVENT_ROUTE_RESOLVED:
                ...
                //LOG_DEBUG("cm_id %p route_resolved ", event->id);
                // if(rdma->callbacks.on_route_resolved)
                //     rdma->callbacks.on_route_resolved(event->id);
                LOG_DEBUG("Route resolved.\n");
                // if(rdma->callbacks.on_connect_before) {
                //     rdma->callbacks.on_connect_before(conn);
                // }
...
void _rdma_wc_handler(void *ctx, int res, int flags){
    ...
    struct ibv_wc wc[10];
    while (1) {
        int ret = ibv_get_cq_event(rdma->comp_channel, &cq, &ctx_wc);
        if(ret == -1) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                // No more events to process
...
static int _kvs_rdma_create_global_resources(struct kvs_rdma_engine_s *rdma, struct ibv_context *verbs) { 
    if(rdma->verbs != NULL) {
        // already created
        if (rdma->verbs != verbs) {
            LOG_FATAL("Multi-device support not implemented!");
            return -1;
        }
        return 0;
    }
    rdma->verbs = verbs;
    if(rdma->verbs == NULL) {
        LOG_ERROR("Failed to get verbs from RDMA context");
        return -1;
    }

    rdma->comp_channel = ibv_create_comp_channel(rdma->verbs);
    if (rdma->comp_channel == NULL) {
        perror("ibv_create_comp_channel failed");
        return -1;
    }

    int flags = fcntl(rdma->comp_channel->fd, F_GETFL);
      if(flags < 0) {
        perror("fcntl F_GETFL failed");
        ibv_destroy_comp_channel(rdma->comp_channel);
        return -1;
    }
    if(fcntl(rdma->comp_channel->fd, F_SETFL, flags | O_NONBLOCK) < 0) {
    ...
    rdma->cq = ibv_create_cq(rdma->verbs, rdma->cq_size, NULL, rdma->comp_channel, 0);
    if (rdma->cq == NULL) {
    ...
    if(0 != ibv_req_notify_cq(rdma->cq, 0)) {
    ...
    kvs_event_init(&rdma->wc_event, rdma->comp_channel->fd, KVS_EV_POLL_IN, _rdma_wc_handler, rdma);
    LOG_DEBUG("Registering RDMA work completion event handler.\n");
    kvs_loop_add_poll_in(rdma->loop, &rdma->wc_event);

    LOG_DEBUG("RDMA global resources created successfully.\n");
...
kvs_status_t kvs_rdma_post_recv(struct kvs_rdma_conn_s *conn, struct kvs_rdma_mr_s *mr, size_t off_set, int len, void *user_data) {
    ...
    LOG_DEBUG("cm_id %p post recv on cq %p", conn->cm_id, conn->cm_id->recv_cq);
    ibv_post_recv(conn->cm_id->qp, &recv_wr, &bad_recv_wr);
